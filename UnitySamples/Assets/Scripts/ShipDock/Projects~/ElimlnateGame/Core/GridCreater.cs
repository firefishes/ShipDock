...
namespace Elimlnate
{
    public class GridCreater
    {
        ...
        public void InitAndCreate(BoardGrids boardGrids, Vector3 gridsOffset)
        {
            ...
            mBoardParent.SetParent(mCore.GridsContainer.transform);
            GameObject shapeParent = new GameObject("shapeParent");
            mGridParent = shapeParent.transform;
            mGridParent.SetParent(mCore.GridsContainer.transform);

            Transform trans = mCore.GridsContainer.transform;
            StartPos = new Vector3(trans.position.x, trans.position.y, 0f);

            float halfToCenter = -0.5f;
            float xOffset = CellSize.x;
            float yOffset = CellSize.y;
            ...
            PosOffset = new Vector3(xOffset * colCount, yOffset * rowCount) * halfToCenter + GridsOffset;

            InitGrids(rowCount, colCount, xOffset, yOffset, ref boardGrids);
        }

        private void InitGrids(int rowCount, int colCount, float xOffset, float yOffset, ref BoardGrids boardGrids)
        {
            Vector3 tileOffset, curPos;
            ...
            for (int row = rowCount - 1; row >= 0; row--)//预制体为非镜像的方式摆放，故使用倒序遍历
            {
                ...
                {
                    tileOffset = new Vector3(xOffset * col, yOffset * row, 0f);
                    ...
                    int rowValue = rowCount - row - 1;//倒序遍历，使用差运算
                    boardGrids.CreateGrid(col, row, newTile);
                    Vector2Int colRow = new Vector2Int(col, row);
                    int index = (rowValue * colCount) + col;
                    int gridType = GetGridTypeDuringCreate != default ? GetGridTypeDuringCreate.Invoke(index) : mCore.GridTypes.NormalGridType;
                    CreateGrid(colRow, curPos, out GridCreateInfo info, string.Empty, true, gridType);
                }
            }
        }
        ...
        private void OnGridInied(ElimlnateGrid grid, GridCreateInfo info)
        {
            ...
            if (InitedGridCount <= 0)
            {
                ...
                {
                    ElimlnateGrid item;
                    int max = mCore.BoardGrids.GridCount;
                    for (int i = 0; i < max; i++)
                    {
                        item = mCore.BoardGrids.GetGridByIndex(i);
        ...
        private GridCreateInfo GetNewGridInfo(int gridType, Vector3 targetPos, string enterEffectName, bool isDirectShow = true)
        {
            ...
            GridType gridPool = mCore.GridTypes.GetGridPool(gridType);
        ...
        private void OnNewGridCreated(INoticeBase<int> param)
        {
            ...
            grid.GridTrans.name = name.Append("@[", row.ToString(), "_", col.ToString(), "] #", grid.GridShapeIndex.ToString());

            if (!mCore.BoardGrids.HasEnoughGrids())
            {
                CreateGridOperateUI?.Invoke(grid);
            }
            else { }
            mCore.BoardGrids.SetGridMapper(new Vector2Int(col, row), grid, false);
        ...
        {
            return mCore.GridTypes.GetRandomGrid();
        }
        ...
        {
            BoardGrids boardGrids = mCore.BoardGrids;
        ...
        {
            ElimlnateGrid grid = mCore.BoardGrids.GetBoardGrid(new Vector2(col, row));

            grid.WillDestroy();

            Vector3 targetPos = mCore.GridCreater.GetGridPosWithRowCol(col, row);
