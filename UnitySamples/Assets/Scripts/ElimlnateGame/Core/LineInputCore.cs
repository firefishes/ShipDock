...
namespace Elimlnate
{
    ...
    public class LineInputCore
    {
        ...
        /// <summary>
        /// 是否为无效的指针抬起输入操作，防止多个消除格输入操作出现的问题
        /// </summary>
        /// <returns></returns>
        private bool IsInvalidPointerUp()
        {
            ...
            {
                int last = count - 1;
                ElimlnateGrid lastGrid = mLinedGridList[last];
                mLastGridTrans = lastGrid != default ? lastGrid.GridTrans : default;
        ...
        private void AddLinedGrid()
        {
            ...
            if (!IsContainsInLined(ref mLining))
            {
                ...
                ElimlnateGrid last = lastIndex >= 0 ? mLinedGridList[lastIndex] : default;
                if (last != default)
                {
        ...
        private void LineEnd()
        {
            ...
            if (mIsLining)
            {
                ...
                bool hasInvalidGrid = max > 0;
                SetResult(hasInvalidGrid);
            }
            else { }
        }

        private void SetResult(bool hasInvalidGrid)
        {
            int count = mLinedGridList.Count;
            if (ShouldInput && !hasInvalidGrid && (count >= mRemoveMinNum))
            {
