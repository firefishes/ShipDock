...
namespace Elimlnate
{
    public class GameEffects
    {
        ...
        /// <summary>消除格被整理时的特效名</summary>
        public static string EffectRemains { get; set; } = "Remins";
        ...
        private bool CheckCreateOrEnterEffectExistable(bool conditionWithFinished, int remainsEnter = 0, int remainsCreate = 0)
        {
            ...
            if (mRemainsEffect == default)
            {
                mRemainsEffect = Effects[EffectRemains];
            }
            else { }

            int enterCount = mEnterEffect.EffectCount;
            int createCount = mRemainsEffect.EffectCount;
